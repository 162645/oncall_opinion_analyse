...
def main() -> None:
    ...
    for i in range(args.count):
        ...
        cases.append({"case_id":case_id,"query":f"分析 {region} {case_type} 网络质量","fixture_path":f"fixtures/{case_id}.json","required_queries":qids,"expected_verdict":"PARTIAL" if case_type=="fault_injection" else "PASS","allowed_facts":["p95_spike_detected","baseline_degradation","asn_concentration","prefix24_candidates","traceroute_paths_observed","rtt_path_time_correlation"],"forbidden_facts":["path_change_caused_rtt"],"case_type":"fault_injection" if case_type=="fault_injection" else "real_data_replay"})
