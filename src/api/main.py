...
from .router import knowledge, chat, agent, health, llm, langgraph, skill, clickhouse
...
Traceroute 是网络路径探测工具，通过利用 IP 报文的 TTL 字段，逐跳探测从源到目的地的网络路径。
...
async def init_default_knowledge():
    ...
    if stats["total_documents"] == 0:
        ...
        for doc_data in DEFAULT_KNOWLEDGE:
            try:
        ...
        print(f"📊 知识库初始化完成: {new_stats['total_documents']} 文档, {new_stats['total_chunks']} 块")
    else:
        print(f"📚 知识库已存在: {stats['total_documents']} 文档")
...
async def lifespan(app: FastAPI):
    """应用生命周期管理"""
    # 启动时初始化
    print("🚀 Starting Oncall Opinion Analyse API...")

    # 初始化默认知识库
    await init_default_knowledge()
...
# 创建 FastAPI 应用
app = FastAPI(
    title="Oncall Opinion Analyse API",
    description="智能运维诊断平台 API 服务",
    version="4.0.0",
    lifespan=lifespan,
    docs_url="/docs",
    redoc_url="/redoc",
)
FastAPIInstrumentor.instrument_app(app, tracer_provider=get_telemetry().provider)

# CORS 配置。生产环境通过 CORS_ORIGINS 注入实际前端域名，避免把开发域名
# 和线上策略硬编码在同一份代码中。
_cors_origins = [
    origin.strip()
...
async def general_exception_handler(request: Request, exc: Exception):
    ...
    # 详细异常写入服务端日志，对外只返回稳定错误码，避免泄露内部连接信息、SQL
    # 或第三方服务响应。
    import logging
    logging.getLogger(__name__).exception("Unhandled request error request_id=%s", request_id)
    return JSONResponse(
...
app.include_router(langgraph.router, prefix="/api/graph", tags=["langgraph"])
...
async def root():
    return {
        "name": "Oncall Opinion Analyse API",
        "version": "4.0.0",
        "docs": "/docs",
        "health": "/health",
    }
