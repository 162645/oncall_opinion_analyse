...
async def list_regions():
    ...
    try:
        ...
        # Region discovery can be expensive on a large import_files table.
        # Keep the demo usable during a ClickHouse resource incident while
        # making the degraded source explicit to the caller.
        fallback_regions = [
            item.strip()
            for item in os.getenv("DEMO_REGIONS", "UKRAINE").split(",")
            if item.strip()
        ]
        return {
            "success": True,
            ...
            "total": len(fallback_regions),
            "source": "fallback",
            "warning": "ClickHouse region discovery is temporarily unavailable; showing demo regions.",
