...
class DiagnosisAgent(BaseAgent):
    ...
    async def execute(self, context: AgentContext) -> AgentResult:
        ...
        knowledge_text = knowledge.get("knowledge", "") if isinstance(knowledge, dict) else ""
...
**根因分析**:
{knowledge_text[:500] if knowledge_text else "需要更多信息进行诊断"}
...
"""
        ...
        return AgentResult(
            agent_name=self.name,
            success=True,
            data={"diagnosis": diagnosis, "confidence": 0.85},
            confidence=0.85,
        )
