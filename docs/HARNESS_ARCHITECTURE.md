...
`src/eval/network_harness_eval.py` 提供可扩展的 Network Analysis Eval Set，统计 TaskSpec Accuracy、Plan Accuracy、Evidence Coverage、Unsupported Claim Rate、Correct Abstain Rate、跨证据一致性、平均轮数和查询数。它评估 Harness 决策纪律，不把没有标注数据支撑的结果伪装成线上准确率。
