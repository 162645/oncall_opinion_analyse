...
前端通过 Nginx 访问 Python 服务；HTTP 和 gRPC 由同一个 Python 运行时提供。主链路为：

```text
Chat API → Understand → Context → Planner → Executor → Verifier → Synthesizer
                                      │          │          └→ Evidence Ledger
                                      │          └→ Query Catalog → ClickHouse
                                      └→ Skills / MCP Resources
```

LangGraph 只负责这条主链路的状态流转；Query Catalog 负责受控查询合同，Tool Runtime/MCP 负责工具边界，Verifier 负责证据门禁，React 只消费最终的 Answer/ChartSpec，不参与 Agent 决策。
...
| Agent Harness | 六节点状态图、受控 Query Catalog、证据校验和可回放 Trace |
...
│   ├── harness/           # 六节点证据驱动 Harness 与 Query Catalog
│   ├── agents/            # 领域服务与兼容接口
│   ├── api/               # FastAPI 路由
...
├── tests/                 # 单元与集成测试
├── biz/                   # Go 告警回调服务
└── proto_gen/             # 由 proto/agent.proto 生成的跨语言代码
```
...
- [Evidence-driven Harness 设计方案](docs/EVIDENCE_DRIVEN_HARNESS_DESIGN.md)
